...
sajat_gyujt = pd.read_csv("d:/Egyetem/01Ma_Survey/Szakdolgozat/kod/Konye-MscCode/preprocessing_train_indep/databases/sajat_gyujtes.csv")

# Mennyi sora van
print(f"Eredeti sorok száma: {len(sajat_gyujt)}")

analyze_text_column(sajat_gyujt, "fake_news")
fake_news = sajat_gyujt[['label', 'text', 'text_length']]
...
print("SpaCy preprocessing pipa")
...
padded_sajat, _, _ = prepare_for_modeling_with_glove(
    tokenized_sajat,
    glove_file=None,  # Nem kell újra betölteni a GloVe-ot
    tokenizer=tokenizer,
    fit_tokenizer=False,
